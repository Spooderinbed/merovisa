...
/**
 * A bordered country-code pill — the neutral, flag-free way to mark a
...
export function IsoPill({ code, className }: { code: string; className?: string }) {
  return (
    <span
      aria-hidden
      className={cn(
        "inline-flex items-center justify-center rounded-md border border-line bg-bg-tint px-1.5 py-0.5 text-caption font-medium uppercase tracking-wide text-ink-soft",
        className,
      )}
    >
      {code}
