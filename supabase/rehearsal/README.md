...
| `MV-185-mutation.sql` | MV-185's MUTATION harness, and a different shape again: it reverts **one** clause of the document-collaboration schema at a time — a policy conjunct, a read policy, or one of the three status triggers — so a **named** test in `tests/integration/stage4-document-collaboration.itest.ts` can be watched going red for the reason it states. Thirteen mutants, each mapped to its test in the file's own header table. Needed because a denial-only RLS suite passes identically against a *missing* policy, so "the probe denied" is not evidence on its own. Restoring is a step, not a rollback: re-run `20260821120000_stage4_case_document_collaboration.sql`, whose §8 re-asserts every guard, so a restore that did not restore refuses instead of passing quietly. **Local only — between mutation and restore the database is deliberately vulnerable.** (`MV-161-mutation.sql` / `MV-161-supersedes.sql` / `MV-161-rollback.sql` are still on disk and still absent from this index; adding them is somebody's follow-up, not this row's.) |
