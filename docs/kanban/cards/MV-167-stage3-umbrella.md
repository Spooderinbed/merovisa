...
- **The access matrix:** spec §4 — **23** surfaces × 7 actors, with a slice-ownership column and a trace-to-canonical column.
...
- **The findings:** spec §7 — **eight**. F-1 (exit gate vs. canonical), F-2 (nobody can create an org), **F-3 (the student's `display_name`/`email` write is a CANONICAL cell a new Stage 3 surface exposes — a founder decision, not a slice's)**, F-4 (internal notes has a permission but no column), F-5 (team management without invitations), F-6 (two mislabelled registry entries), F-7 (confirmed-correct, do not re-litigate), **F-8 (five case-scoped write routes resolve the ACTOR's own case and take no case id)**.
- **Two founder decisions stand open, not one: F-1 and F-3.** Neither is an agent decision. F-1 gates MV-171; F-3 gates nothing in the build — the stage ships without it under reading (a) — but it must not be closed by a slice.
...
|---|---|---|---|---|
| **MV-168** CONSULTANCY WRITE GRANTS | prereq | The only SQL in Stage 3: `INSERT` grant+policy on `profiles` and `plan_items`, `UPDATE (is_primary)` grant+policy on `assessments`, each mirroring the five-sibling `WITH CHECK` template. **Plus the three `.upsert()` → read-then-insert conversions the grants depend on** — `upsertProfileForCase`, `upsertProgramState`, `setObtained` (spec §6.1, F-8). Retires 2 of the 4 DEFERRED HALF `42501` assertions and rewrites the other 2's comment. Amends Stage 2 spec §6. | — | No UI, no route **signature** changes (MV-172). No `assessments`/`documents` INSERT (refused / deferred to Stage 4). No schema change. |
...
| **MV-172** THE CASE ROUTE | 3 | Render the existing MeroVisa experience under an explicit case route for a case that is not the actor's own. Flips `app/api/plan/action` onto the authenticated client and **splits** `app/api/profile/section` — profile write moves, three refused legs stay service-role. **Plus F-8**: the five case-scoped write routes take an explicit case id. | MV-168, MV-171 | No documents model change (Stage 4). No indicators (MV-173). **Does not retire `profile/section`'s registry entry.** |
| **MV-173** CASE-CONTEXT INDICATORS + FIELD ALLOWLIST | 4 | Whose case am I in, and is it mine — persistent and unmissable. Plus the **TypeScript** field allowlist (`lib/cases/permissions.ts` must stop admitting an arbitrary student case patch — `lib/cases/README.md:152-157`). | MV-172 | No new data. No notes (F-4). **Not F-3's column guard** — canonical amendment, founder's call. |
| **MV-174** SERVICE-ROLE RETREAT + STAGE EXIT | exit | Reclassify registry entries 1–2, narrow 3–4 **and 9**, confirm 5–7 wait for Stage 4, register the new scoring route — ending at **nine**. Prove the exit gate. **Carries the stage exit.** | all | No new surfaces. |
...
- [ ] **The stage exit gate is observably green (MV-174).** **Seven** criteria E1–E7, spec §9.1, each with its vacuity guard from §9.2 actually implemented — notably **E4 must assert `cases.student_user_id IS NULL`, `owner IS NULL` on every row written, *and* a created-row count `> 0`**, or it passes trivially on an empty write set (the direct analogue of the Stage 2 §A2 vacuity MV-165 recorded); **E4's seam half must read the row back**, because `setObtained`/`upsertProgramState` return `false` rather than throwing; and **E7's counsellor fixture must itself hold a personal case with a row on the same table**, or a wrong-case write has nowhere to land and the test proves nothing.
...
- [ ] **The service-role list is accounted for entry by entry, and its growth is admitted.** 9 → 8 by disposition (**1 retires** — `plan/action`; **3 narrow** — `assess/refresh`, `assess/route`, **`profile/section`**; 2 reclassify; 3 wait for Stage 4) **plus 1 new entry** for the case-scoped scoring route → **9 at stage exit, the same count it started with**. MV-154's framing of the list as monotonically shrinking does not survive Stage 3. **`app/api/profile/section/route.ts` does NOT retire** — `assessments` UPDATE `(result)`, `plan_items` copy refresh and `adoptOwnerKeyedResidue` are all refused by spec §6.1, and all three fail *silently* if flipped. The exit records the new entry as a deliberate registration rather than a leak, and scores the stage on scope, not on list size.
...
- **MV-168** — real-DB: a counsellor INSERT into `profiles`/`plan_items` for an assigned case **succeeds with `owner IS NULL`**; the same INSERT naming another user's id as `owner` is refused; the `assessments` and `documents` INSERT refusals stay `42501`; the DEFERRED HALF test's remaining assertions still pass with a corrected comment. **Plus: the authenticated client creates a first-ever `profiles` row — a test that FAILS against the `.upsert()` form** — and `setObtained` / `upsertProgramState` succeed on a case with `student_user_id IS NULL` (today `caseUpsertColumns` refuses it outright).
...
- **MV-172** — route renders for the assigned counsellor, 404s for the unassigned; `plan/action` no longer constructs a service-role client (the ESLint fence + registry sweep are the mechanical check), while `profile/section` **keeps its entry** with a narrowed justification. **Plus F-8**: each of the five case-scoped write routes writes a row carrying the **student's** case id, read back — with the counsellor holding their own personal case in the fixture so a wrong-case write is observable.
- **MV-173** — the indicator names the case and distinguishes "not mine"; the TypeScript allowlist rejects a student case patch carrying a non-permitted field. **Not F-3's column guard.**
- **MV-174** — E1–E7 with the §9.2 guards; the registry disposition test asserts **per entry**, never a count.
...
- **F-3 is a real trust risk AND a canonical cell — do not let a slice close it.** A student can rewrite the `display_name` and `email` that MV-170's list will show a counsellor. But that write is the **canonical** student surface (`stage1-canonical-access-matrix.md:68-70`; `20260730180000….sql:459`), pinned positively at `tests/integration/case-rls.itest.ts:833`. Narrowing it turns a Stage 1 test red, which spec §5 defines as proof a slice moved a cell and is wrong. **Founder decision. Mitigation under reading (a) is at MV-170's display layer, not the write layer.**
- **A green suite will not prove the stage.** Spec §9.2 lists, per criterion, how it can pass while asserting nothing. E4 is the one that repeats Stage 2's §A2 failure if written carelessly — and **F-8 is the one that ships a cross-case write under a green gate**: the five routes already hold grants and policies, so a mis-scoped write *succeeds* rather than erroring.
- **The service-role list does not shrink this stage.** 9 in, 9 out. A reader scoring Stage 3 on that number will conclude it achieved nothing; the advance is in scope (writes move onto the authenticated client, and the case route reaches student data as the authenticated user for the first time), not in count.
...
2. Check whether F-1 has been decided. If not, MV-168/169/170 are still safe to build; MV-171 is not. **F-3 is a second open founder decision — it blocks no slice, but no slice may close it.**
...
- **2026-08-07 — carved by MV-166.** Seven slices from four plan bullets, grounded in a live schema read rather than the prose. Key carve decisions: the grants are a **prerequisite slice** (MV-168), not a late one, because nothing can be written into a student-less case without them; bullets 3 and 4 stay separate because the indicator needs the route to exist first; and the exit gets its own slice because the service-role retreat and the exit proof are the same work.
...
- **2026-08-07 — this card trued against the revised spec (same PR).** Four spec corrections landed after adversarial review and each changed something here: `profile/section` **narrows rather than retires** (three refused legs, all failing silently) so the registry ends at **9, not 8**; **grant 1 needed its `.upsert()` call site converted** or it never reaches its caller, which moves three conversions into MV-168; **F-8 added** — five case-scoped write routes resolve the actor's own case, so MV-172 gains the route-signature work and the gate gains **E7**; and **F-3 reclassified** from a MV-173 build item to a second open **founder** decision, because narrowing the student's `display_name`/`email` write moves a canonical cell. Slice map, acceptance criteria, test plan and risk notes updated accordingly.
