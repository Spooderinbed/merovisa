...
- 2026-07-31 — **The TypeScript layer is probed with the SERVICE-ROLE client injected as `getCaseContext`'s `db` argument; the database layer is probed only through an RLS-scoped client.** This is what makes the two assertions independent. `getCaseContext` uses the authenticated client in production, so probing TS through the actor's RLS client would let RLS silently answer for it — a TS matrix bug would hide behind a correct policy and the harness would report agreement. Injecting the unfiltered client asks the question that matters: *if RLS were bypassed, does TypeScript still deny?* It is the only service-role client in an assertion path. (Stale round-1 wording corrected 2026-08-02: this line said "`clientForUser`", a helper the fixture never shipped under that name, and "all 17 actor JWTs" — there are 21, and as of round 2 the self-check asserts a behavioural property of each CLIENT rather than only the `role` claim on its token.)
...
### Round 3 — an independent two-lens audit verified all six round-2 items closed, and found one more real gap plus some overstated prose.

- 2026-08-02 (r3) — **MAJOR: `organization_memberships` UPDATE and DELETE were recorded as covered but only probed where the probe is structurally blind.** Exactly the class of gap BLOCKER 1's `organizations` DELETE was, and the remedy built for that one was not applied here. Every client-issued membership write in all three itests is either a cross-tenant attacker (who holds no membership in the victim org) or a REVOKED member on their own row — and `actor_org_ids()` excludes both, the first for having no membership there at all, the second by the status filter. Widening `organization_memberships_update_admin` / `_delete_admin` from `actor_admin_org_ids()` to `actor_org_ids()` was therefore invisible: an ordinary active counsellor gaining the power to delete the owner's membership row would not have turned the exit gate red. The audit also found the gap slightly wider — `case-rls.itest.ts`'s test titled "lets only an owner mint or change an owner membership" contains two INSERTs and no UPDATE, so the `role <> 'owner'` carve-out on `organization_memberships_update_admin` (migration 20260730180000 lines 373-383) was asserted for INSERT only. Three new tests, against disposable orgs for the same blast-radius reason as tenant destruction: an active non-admin member refused every roster write on rows they can plainly see; an active admin allowed to suspend and evict a counsellor but refused an owner row on UPDATE and DELETE and refused self-promotion to owner (the WITH CHECK half); and an owner allowed all three, so none of it is "nobody can". Two new fixture actors, `rosterProber` and `rosterOwner`, hold no org A/B membership so they add no matrix cells.
- 2026-08-02 (r3) — **FALSE CLAIM corrected by making the mechanism real, not by softening the prose.** The card, PR body and PR comment all claimed `GRANTED_WRITE_VERBS` made coverage self-policing — "a new grant without a new assertion fails a test". The verb-level transcription was exact, but it was a hand-written literal in the same file as the probes, compared only against the probes' own `record()` calls; nothing read the catalogue. Add a grant to the migration and both sides stayed untouched and green. (The outcome happened to hold only because `case-rls.itest.ts` independently pins `information_schema` with an exact `toEqual` — a different suite, and a mechanism this card never named.) `readGrantedWriteSurface()` now reads the live grant set out of `information_schema` at run time via psql, the same idiom `case-rls.itest.ts` uses because `information_schema` is not a PostgREST-exposed schema. **This is the second load-bearing claim on this card that overstated what an artifact proved** (the 292/292 inversion was the first); both are now backed by a mechanism that fails when the claim stops being true.
- 2026-08-02 (r3) — **The completeness guard is column-granular for UPDATE, because the grants are.** A verb-level guard collapsed `cases` UPDATE(display_name, email, operational_status, archived_at) into one key and was blind to a column being added to an existing verb. `cases.email` was the live instance: granted, and probed by nothing — the column that carries a real student's address from Stage 3 on. The catalogue now attempts each granted column in its own statement, and the derived key set is `table.insert` / `table.delete` / `table.update(column)`. `column_privileges` also expands a table-level UPDATE grant into one row per column, so `grant update on public.cases` would demand a probe per column — loud, and correct for a change that size.
- 2026-08-02 (r3) — **Four smaller corrections, all verified by the audit.** (a) `cloneCase`'s justification was imprecise in the same way BLOCKER 2's original was: `archived_at` and `operational_status` are NOT copied, so "unset on both" was wrong. The conclusion holds for a structural reason instead — `enforce_case_write_surface` is a BEFORE **UPDATE** trigger, so it never fires on `cases` DELETE and does not exist on `case_assignments`, and no delete/assign policy predicate reads either column. (b) The comment claiming the self-check covered the tampered-JWT client was false: registering a client is not asserting anything about it, and that client is registered *after* the opening self-check has run. A closing self-check now re-runs the property over the full registry and asserts the registry has grown and contains it. (c) `inOrgAssignee` handed the assign probe the actor's own id for `counsellorUnassignedA`, breaking the probe's stated "never the actor" invariant and quietly turning two cells into the different question `case-rls.itest.ts` owns. (d) The self-check's client property is one-sided — "sees fewer than all seeded cases" excludes a BYPASSRLS client but also passes for an under-privileged one — so the limit is now stated and a lower bound (at least one client sees rows) keeps it from being vacuous in aggregate.
- 2026-08-02 (r3) — Two stale round-1 references in this log corrected: `clientForUser` (a helper name the fixture never shipped) and "all 17 actor JWTs" (21, and the self-check no longer asserts only the token claim).
...
### Files
- `tests/integration/tenant-isolation.itest.ts` — the matrix (429 tests).
- `tests/integration/fixtures/tenancy.ts` — two-org fixture factory, the recording RLS-client factory, localhost hard-guard, disposable-org seeder, `readGrantedWriteSurface()` (the live `information_schema` read behind the completeness guard), and the probe machinery (one probe per verb, each returning a boolean plus the evidence behind it).
...
### Gate — all green, 2026-08-02 (round 3)
| Gate | Result |
|---|---|
| `npm run typecheck` | clean |
| `npm run lint` | clean |
| `npm test` | **2470 passed** / 323 files |
| **`npm run test:integration`** | **528 passed / 5 files / 0 skipped** (local stack, all migrations incl. `20260730120000` + `20260730180000` applied) |
| of which this suite | **429 passed** |
...
### Mutation evidence — the assertions bite, and the previous round's did not
Each mutation is applied to the local database, the suite is run, the mutation is reverted. Every one is also run against the PREVIOUS round's test files, unchanged, for the counterfactual.

| Mutation | round 1 | round 2 | round 3 |
|---|---|---|---|
| `private.org_role`: drop `and m.status = 'active'` | 341/341 **pass** | **12 fail** | — |
| `organizations_delete_owner`: `actor_owner_org_ids` → `actor_admin_org_ids` | 341/341 **pass** | **1 fail** | — |
| `cases_delete_admin`: add `or student_user_id = auth.uid()` | 341/341 **pass** | **8 fail** | — |
| `private.can_manage_case`: add assigned counsellors | 341/341 **pass** | **5 fail** (via the roster-split FINDING) | — |
| `organization_memberships_update_admin`/`_delete_admin`: `actor_admin_org_ids` → `actor_org_ids` | — | 425/425 **pass** | **1 fail** (`holds the roster shut against an active, in-tenant, non-admin member`) |
| the same two policies: drop the `role <> 'owner'` carve-out | — | 425/425 **pass** | **1 fail** (`lets an admin run the roster but never touch an OWNER row`) |
| `grant update (status) on public.organizations` — a grant nobody probes | — | 425/425 **pass** | **1 fail** (`attempted every write verb the LIVE GRANT SET actually hands authenticated`) |

The last row is the proof that the completeness guard is now a real tripwire rather than a claim: the same mutation left round 2 fully green because both sides of its comparison were hand-written in the test file.

Baseline and post-revert runs are 429/429 green at every step, so no mutation leaked.
...
| **WRITE denial — every granted verb, six tables, both directions; tenant destruction and roster control by in-tenant probers; + the live grant-coverage assertion** | 9 | — | ✔ |
...
| Harness self-check (21 actor JWTs; every issued CLIENT is RLS-filtered; `createClient` call sites pinned; every case shape seeded; anon holds nothing) | 5 | — | ✔ |
| Harness self-check, closing (the full client registry, including clients a single test built) | 1 | — | ✔ |

Cross-tenant write verbs asserted, in both directions: `organizations` UPDATE(name) + UPDATE(slug) + DELETE · `organization_memberships` INSERT + UPDATE(role) + UPDATE(status) + DELETE · `cases` INSERT + UPDATE(display_name) + UPDATE(email) + UPDATE(operational_status) + UPDATE(archived_at) + the tenant-move + DELETE · `case_assignments` INSERT + DELETE · `invitations` INSERT (team and student shapes) + UPDATE(revoked_at) · `audit_events` INSERT + UPDATE + DELETE (no grant at all). The set is compared for equality against the grant set read out of `information_schema` **at run time**, column-granular for UPDATE, so it cannot drift from the migration in either direction — a new grant fails the test until it is probed, and a probe for a verb nobody holds fails too.

In-tenant probers, where a cross-org probe is structurally blind (the attacker holds no membership in the victim org, so the org-id comparison refuses them whichever actor set the policy names): `organizations` DELETE by a counsellor, an admin and a revoked owner of the org itself, with the real owner's delete asserted to succeed; `organization_memberships` UPDATE and DELETE by an active non-admin member, by an admin against an owner row (both verbs) and against their own row promoted to owner, with an owner allowed all three.
...
- **`case.delete` and `case.assign` probe a CLONE, not the fixture row.** A delete that unexpectedly succeeded against the real row would take the fixture down and bury the finding under hundreds of unrelated failures. The clone copies `organization_id`, `student_user_id` and the assignment roster — every fact a delete-path or assign-path predicate reads — which mutation M3 confirms behaviourally. It does **not** copy `archived_at` or `operational_status`; the reason that cannot matter is structural rather than incidental (an earlier draft of this line said "unset on both", which was wrong): the only Stage 1 code that reads either column for an authorization decision is `enforce_case_write_surface`, a BEFORE **UPDATE** trigger, which never fires on `cases` DELETE and does not exist on `case_assignments`. A future "archived cases cannot be deleted" rule would invalidate this and the clone would have to start copying them.
- **The self-check's client property is one-sided.** "No issued client sees every seeded case" excludes a BYPASSRLS client, but would equally pass for an under-privileged one — a client whose token was rejected outright sees zero and satisfies it. A lower bound (at least one client sees rows) keeps it from being vacuous in aggregate; the per-actor row sets that would actually catch an under-privileged client are the `case.list` block's job.
