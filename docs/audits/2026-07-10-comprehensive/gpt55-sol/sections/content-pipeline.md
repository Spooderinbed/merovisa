...
### P2 — The seeded DB catalogue (15 universities + 83 programs) is dated but no freshness mechanism walks it

The base seed migration stamps `last_verified 2026-06-04` on 15 universities + 64 programs, and the later fact-layer bridge expands the catalogue to **83 programs**. The general freshness guards import `DATA_MODULES`; they do not walk Supabase catalogue rows. Parity tests catch TS↔SQL divergence, but there is no `reverify_by` field or scheduled job that asks when each provider requirement or tuition value must be checked again. The catalogue that drives match tuition/requirement gaps can therefore become old while structural/parity tests remain green.
...
`au-cost-of-living.ts:118` describes 44,500 as the representative median tuition from the earlier 64-program seed, but the catalogue now contains 83 programs. No test recomputes the statistic from the current catalogue. Add or reprice programs and this silently diverges from its own definition, moving the capacity gate for everyone with no `CONFIG_VERSION` change.
...
Marketing lists six intended destinations while `SUPPORTED_DESTINATIONS` correctly enables only Australia; placeholder cost/English/currency rows already create a false sense of readiness. At even 3 destinations × 3 source countries the fact surface becomes many times today's, and re-verification becomes a **year-round stream of overlapping deadlines from different policy and financial-year calendars**. The founder-manual model breaks at exactly the point the expansion thesis begins, because:
...
```mermaid
flowchart LR
  A["Discover official change"] --> B["Capture immutable source snapshot + hash"]
  B --> C["Parse into typed staging facts"]
  C --> D["Diff against published version"]
  D --> E["Risk, volatility, confidence, impacted outputs"]
  E --> F["Human review; two-person approval for high-stakes rules"]
  F --> G["Publish immutable bundle with effective dates"]
  G --> H["Invalidate caches and recompute affected journeys"]
  H --> I["Public change log + targeted user notification"]
  I --> J["Freshness monitor and correction SLA"]
```

Every published fact should carry: source URL and source type; captured snapshot/hash; accessed/verified date; effective-from/to; volatility and next review; confidence and derivation method; reviewer/approver; supersedes/superseded-by; and the rules, courses, or user journeys it can affect. Automation may discover, fetch, parse, and propose diffs. It must not auto-publish visa, eligibility, fee, deadline, or ranking changes. The first admin surface should therefore be a review queue and diff viewer, not a free-form CMS.
