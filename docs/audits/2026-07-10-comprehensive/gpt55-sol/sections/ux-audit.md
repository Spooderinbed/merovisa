...
| Surface | What exists | Student-level audit |
|---|---|---|
| `/` | Strong no-signup proposition; sample verdict/plan/checklist/guide/freshness widgets | Samples are hard-coded and several “See…” links start assessment instead of opening the promised sample. Claims about citations, memory, and freshness overstate current behavior. |
| `/how` | Methodology and honest “uploads are not read” disclosure | Conflicts with `/trust`; also implies every verdict is published-threshold-derived and full-course costs are used when internal heuristics/first-year approximations exist. |
| `/trust` | No-commission, storage, security, deletion, correction story | Highest-risk screen: AI/analytics processors, document verification, deletion/retention, hosting, and breach claims do not match implementation or evidence. |
| `/destinations` + `[id]` | Six editorial destination cards/pages | “Six countries, done well” is incompatible with Australia-only assessment; unsupported country figures create freshness cost without an actionable flow. |
| `/auth` | Google OAuth | Direct visitors see “Save your result” even without a result; no real alternate method, busy state, or surfaced OAuth recovery error. |
| `/assess` | Persisted progressive wizard + recap | Calm and localised, but forced non-choices/defaults can become asserted facts; no final review/edit; refusal/dependant inputs missing; budget wording and scoring diverge. |
| `/assessment/[id]` | Anonymous/owned result recovery | Useful result before signup; no direct review/edit. Expired/invalid links fall to a default 404; focused chrome limits signed-in navigation. |
| `/dashboard` | Verdict, readiness, journey, next step, outcome funnel | Good overview but no deadlines/alerts/recent changes; plan mirrors user-entered state and ends at visa decision. |
| `/profile` | Structured editors | Cannot model repeated qualifications/jobs/sponsors/refusals well; several cleared fields are omitted from patches and therefore remain stored. Accuracy meter cannot reach its labels. |
| `/matches` | 83-program cards, bands, costs, scholarships, shortlist/applied | No search/filter/compare/saved-only/deadline/campus detail. Partial profiles become zeroes; all-in budget is compared only to tuition. Applied is high-consequence with weak confirmation/error semantics. |
| `/plan` | Nepal-specific grouped actions | Strong content, but future phases appear early, lack due dates/dependencies, and end at visa decision. Some upload actions imply verification the vault does not perform. |
| `/checklist*` | Overall and program checklists | Valuable but absent from primary navigation. Global list is the full taxonomy, not the union of active requirements; “everything this program requires” overclaims a rule-generated list. |
| `/documents` | Private image/PDF upload/view/delete | A one-file-per-kind locker, not verification/review/packaging/submission. Replacement can delete the old file before the new one validates. Copy still says “photos.” |
| `/guide` | Authenticated, context-grounded DeepSeek chat | Ephemeral, plain-text, no validated citation rendering/feedback/persistence; successful replies are not announced to screen readers; processor disclosure is absent. |

## P0 — UX truth and recovery failures

- Fix `/trust`/`/how`/plan/document copy before another feature: students cannot form an accurate mental model of what uploads, AI, deletion, and “verified” mean.
- Do not render match gaps until grade, English, level/field, and budget semantics are known. Unknown is a state, not zero.
- Split tuition budget, living capacity, and total first-year funds; show the exact source and what is excluded.
- Surface OAuth callback errors and preserve a recoverable assessment rather than redirecting to an inert query parameter.
- Use test-aware PTE/TOEFL component scales and redesign the impossible “profile accuracy” meter as explicit completeness.
