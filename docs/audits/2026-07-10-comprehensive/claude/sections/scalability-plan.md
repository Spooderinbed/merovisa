...
A new *destination* (Canada is the obvious market pull, given the marketing copy already teases it) is **8–12+ weeks**: new bespoke capacity gate, ~30 destination content modules from scratch, new program catalogue, sourced English/cost tables, plus the source-side de-hardcoding. Do it second, after India → Australia proves the source-country seam.

### Marginal cost of corridor #2 (India → Australia) under current architecture

| Work item | Effort | Note |
|---|---|---|
| Un-pin `from-sections.ts` homeCountry/gradeSystem; carry home country on profile | S | Also needed for corridor theming to resolve per-user |
| India `SourceCountryData` + banks/test-centre/funds modules | M | Mirror `nepal-*.ts` structure |
| `au-india-evidence-levels.ts` (DHA evidence levels for Indian passports) | M | **The only hard research task** |
| Per-source grade normalization (India CGPA-10 / percentage conventions) | S | Enum already present; verify the linear map |
| `CorridorId` gains `"in-au"`; theming + mascot variant | S | Registry already supports it |
| Extend `SUPPORTED_DESTINATIONS`? No — India is a *source* change; destination stays AU | — | No scoring-gate change needed |
| Tests (goldens per source country) | M | Golden-assessment discipline already exists |

**Estimate: ~2–4 weeks**, dominated by the India-evidence research and content authoring, *not* engineering — because the scoring engine and destination content are reused wholesale. Contrast with 8–12 weeks for any new destination. That asymmetry is the single most important scaling fact in this codebase, and it argues strongly for a **source-first expansion sequence** (India, then Bangladesh/Pakistan — all reusing the AU destination) before opening a second destination.
