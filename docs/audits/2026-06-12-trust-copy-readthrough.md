...
> **④·3c status (2026-06-12, commit `c33bb8b`): APPLIED — F5 · F6 · F8 · F9, as proposed, plus two user-approved extensions: "travel" added to the F8 body's first sentence (A.011–A.013; matches the checklist finance note), and the F9 defect class swept on the marketing `/how` page ("the financial floor (AUD 29,710 living costs plus first-year tuition)" → "the financial-capacity rules (AUD 29,710 per year for living costs, plus travel and first-year tuition evidence)"). Every changed line is copy-locked.**
> **④·3d status (2026-06-12, commit `0a41a96`): APPLIED — F16, payload-carried. User guardrails honored: no scoring change, no RULE_VERSION/CONFIG_VERSION bump, goldens byte-identical; the card renders "Assessment rules verified {date}" from the scoring config's oldest dated provenance entry; no version jargon rendered; a structural test proves the card no longer reads `destination/australia`. ALL 16 F-ROWS RESOLVED. Watch items stay watch. F2 Option A queued as a research task: properly source the Nepal/SSVF financial-evidence scrutiny claim (see the PROJECT_STATUS backlog).**
> **④·3e status (2026-06-12, commit `e6dd057`): F2 CLOSED — Option A applied per the research brief (`docs/research/2026-06-12-nepal-ssvf-financial-scrutiny.md`). a1 shipped: the banner's gov DCT-pointer line ("DHA's Document Checklist Tool shows exactly what to attach for your passport country and provider.", C.146 `used`). a2 declined for now (DHA does not publish evidence levels — C.147; sector dates wobble — C.145 → `use-later`). b: seasoning stays our-recommendation voice permanently (C.148 → `use-later`, the recorded justification). c: >AUD 5,000 · "DHA case officers now expect" · "Assessment Level" user-facing wording permanently rejected — pinned absent by negative copy-locks on the banner + plan + checklist tests.**
...
- ④·3e note (2026-06-12, `e6dd057`): **Option A resolved — F2 permanently closed.** The research brief found the EL2→EL3 event real but sector-only by design (DHA told The PIE News levels are "not made public"), seasoning permanently guidance (practitioners disagree 3/6/12), and >AUD 5,000 sourceless at every tier. Shipped: the gov DCT-pointer banner line (C.146 `used`). Declined for now: the dated sector-attributed line (C.145/C.147 remain in the ledger as the recorded justification; C.145 `use-later`, revisit ~Sept 2026). The c) wording above is pinned absent by negative copy-locks; this row does not reopen.
...
- ④·3d: applied payload-carried — new `CONFIG_RULES_VERIFIED` (oldest dated `CONFIG_PROVENANCE` entry, currently 2026-06-02) stamps `AssessmentPayload.rulesVerified` in `assembleAssessment`; the card renders "Assessment rules verified {date}" from an optional prop. Direct import was rejected: `results.tsx` is `"use client"`, so importing scoring-config from the card would bundle the rules into client JS. No host shown (no single domain covers the mixed inputs — factor-level `SourceLine`s carry the links) and no version string (internal jargon; the engine already stamps ruleVersion/configVersion into the stored result). Legacy stored payloads render no line (absent beats wrong). Locks: derivation invariants + assemble wiring pin + render/legacy/structural no-`destination/australia` tests.
