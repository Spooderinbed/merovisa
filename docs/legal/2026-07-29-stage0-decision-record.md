...
*The pack itself merged to master as PR #95 on 2026-07-27. This record is the follow-up that
closes it, and it corrects two findings in the pack that the intervening week overtook — see D-A
work items 3 and 8.*
...
3. **A transactional sender for the invitation email.** ⚠ *The pack's "no email capability exists
   anywhere" finding is now partly overtaken.* MV-147 shipped **email sign-in** (magic link / OTP)
   to master on 2026-07-27, closing audit F-6. But that is **Supabase Auth email**: it still needs
   configuring on the hosted project (`docs/email-auth-setup.md`), and GoTrue's built-in sender is
   rate-limited and not intended for product mail. The invitation email carries an APP 5 privacy
   notice and is product mail. So the blocker is reduced, not closed — from "nothing exists" to
   "the auth leg exists; the product leg does not."
...
8. Email sign-in for non-Google students — ✅ **shipped** (MV-147, merged 2026-07-27), pending
   hosted-project configuration. The pack's caveat that invited students may be unable to claim
   their case is resolved once that configuration lands.
...
| #7/#12 | Fate of an unclaimed case (delete or anonymise, after how long) | Retention schedule. **Precedent, not answer:** `docs/data-retention-policy.md` landed on master 2026-07-27 but governs *unclaimed anonymous assessments only* and says so explicitly. Its purge-job + daily cron + never-remove-this-guard pattern is the shape a consultancy unclaimed-case rule should follow |
