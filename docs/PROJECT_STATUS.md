...
**Snapshot:** 2026-06-07, scorer-wiring slices 1–3 + Phase A cost-to-apply + Phase B (B1 visa-English floor + B2 dependents capacity, incl. signed-in family.situation mapping + family child-count field) merged
**Tests:** 657 passing across 156 test files
**Typecheck:** clean
...
- **Open backlog:** `/matches` sourced-data consolidation (single source of truth); trust-sensitive findings (agent fees/KPIs, grad salaries — not sourced, needs caveats); pre-existing ESLint debt.
