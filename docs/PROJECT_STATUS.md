...
- **Open backlog:** next-slice decision from the ranked cluster report (GS module recommended); analytics slice awaiting spec approval (`docs/superpowers/specs/2026-06-10-analytics-instrumentation-design.md`); data-governance Phase 3 (volatile refresh incl. the 15 stale findings + reverifyBy backfill, human read-through packet); the 104-finding human queue (agent-risk editorial task now precisely scoped, salary-evidence policy, gap markers to commission or close); `/matches` Problem B remaining sub-lanes (research-gated); minor parked items: snapshot-card test fixture uses a drifted dimension shape (test-hygiene), `not-sure` wording in factor copy reads "your destination" via the render seam.
