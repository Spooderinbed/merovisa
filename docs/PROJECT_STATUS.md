...
**Snapshot:** 2026-06-07, scorer-wiring slices 1–3 + Phase A cost-to-apply + Phase B (B1 visa-English floor + B2 dependents capacity, incl. signed-in family.situation mapping) merged
**Tests:** 643 passing across 156 test files
**Typecheck:** clean
...
- **Open backlog:** `/matches` sourced-data consolidation (single source of truth); a child-count field on the signed-in family section (the enum currently floors spouse-and-kids at 1); trust-sensitive findings (agent fees/KPIs, grad salaries — not sourced, needs caveats); pre-existing ESLint debt.
