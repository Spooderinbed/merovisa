...
- **Open backlog:** UX-fix order remainder per `docs/audits/2026-06-10-visual-audit-and-fix-order.md` — next #4 plan polish (honest impact claims, Done/Dismissed visuals, counts — builds on the completion model), then #5 checklist step completion (extend the model to STEP rows), #8 remainder (profile-strength empty expander + casing), #10 profile editor consolidation (needs a mini-spec + approval); unlayered-CSS-reset layering (spawn-task chip, needs visual pass); trust-sensitive findings (agent fees/KPIs, grad salaries — not sourced, needs caveats); `/matches` Problem B remaining sub-lanes — deepen sources (derived→primary, deep links) and/or widen coverage from `au-*` (both research-gated).
