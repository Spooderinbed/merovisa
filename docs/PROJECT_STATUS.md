...
- **Open backlog:** UX-fix order: only **#10 profile editor consolidation** remains (13 sections → ~8; editor-level regroup with storage keys stable — no migration, scoring mapping untouched; fold in the parked destination-editor honesty fix — it still lists all six destinations and uses `"us"` where scoring says `"usa"`; needs spec + approval first); unlayered-CSS-reset layering (spawn-task chip task_ee593275 — `p`/`h1–h4` margin resets still eat `mt-*` utilities, needs a visual pass); trust-sensitive findings (agent fees/KPIs, grad salaries — not sourced, needs caveats); `/matches` Problem B remaining sub-lanes — deepen sources (derived→primary, deep links) and/or widen coverage from `au-*` (both research-gated); ledger triage resumes after (F slice-shaped, G needs human sourcing).
