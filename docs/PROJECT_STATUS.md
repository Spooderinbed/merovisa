...
- **Open backlog:** UX-fix order: **#10 profile editor consolidation** in flight (user-approved 13→8 grouping; spec `docs/superpowers/specs/2026-06-10-profile-editor-consolidation-design.md` — editor-level regroup with storage keys stable, chip inputs + grade-system select, destination-editor honesty + `"us"`→`"usa"` shim folded in); trust-sensitive findings (agent fees/KPIs, grad salaries — not sourced, needs caveats); `/matches` Problem B remaining sub-lanes — deepen sources (derived→primary, deep links) and/or widen coverage from `au-*` (both research-gated); ledger triage resumes after (F slice-shaped, G needs human sourcing).
