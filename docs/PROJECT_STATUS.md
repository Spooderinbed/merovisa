...
- **Open backlog:** data-governance Phases 2–3 (cluster triage + analytics, then volatile refresh + read-through packet — see memo); trust-sensitive findings (agent fees/KPIs, grad salaries — not sourced, needs caveats); `/matches` Problem B remaining sub-lanes — deepen sources (derived→primary, deep links) and/or widen coverage from `au-*` (both research-gated); minor parked items: snapshot-card test fixture uses a drifted dimension shape (test-hygiene), `not-sure` wording in factor copy reads "your destination" via the render seam.
