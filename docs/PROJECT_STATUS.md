...
- **Open backlog:** trust-sensitive findings (agent fees/KPIs, grad salaries — not sourced, needs caveats); `/matches` Problem B remaining sub-lanes — deepen sources (derived→primary, deep links) and/or widen coverage from `au-*` (both research-gated); ledger triage resumes (F slice-shaped, G needs human sourcing first); minor parked items: snapshot-card test fixture uses a drifted dimension shape (test-hygiene), `not-sure` wording in factor copy reads "your destination" via the render seam.
