...
**Snapshot:** 2026-06-07, scorer-wiring slices 1–3 + Phase A cost-to-apply panel merged
**Tests:** 614 passing across 155 test files
**Typecheck:** clean
...
- **Next: Phase B recalibration epoch** (approved roadmap-execution plan; batched to pay the golden-regen tax once — one design pass, `RULE_VERSION→v0.3.0`/`CONFIG_VERSION→config-v3`, commit-granular goldens). **B1 — visa English floor:** distinguish the DHA *visa* floor (IELTS 6.0 each band, sourced) from the *course* threshold (6.5) in `visa.ts`; the real over-penalty is ~5 visa pts at 6.0 (10 pts/band — narrower than first billed), mostly a factor-narrative fix. **B2 — dependents → DHA capacity:** the financial gate adds partner/child capacity (AU figures already sourced: partner 10,394 / child 4,449 / school 13,502); build on the existing `family.situation` enum rather than a new field. Both verdict-changing → one brainstorming spec before any engine code.
...
- **Open backlog:** Phase B (above); `/matches` sourced-data consolidation (single source of truth); trust-sensitive findings (agent fees/KPIs, grad salaries — not sourced, needs caveats); pre-existing ESLint debt.
